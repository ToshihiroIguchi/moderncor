...
available_methods <- function() {
  methods_list <- list(
    ...
    list(method = "biweight", label = "Biweight Midcorrelation", package = "built-in", type = "robust"),
...
method_info <- function(method) {
  ...
  info <- switch(method,
    ...
    biweight = list(
      ...
      package = "built-in",
      description = "Robust correlation measure based on biweight (bisquare) weights, insensitive to outliers.",
      range = "[-1, 1]",
      assumptions = "Continuous variables."
    ),
